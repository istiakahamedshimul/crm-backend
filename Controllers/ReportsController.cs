...
public class ReportsController(CrmDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult> Workspace([FromQuery] DateTime? from = null, [FromQuery] DateTime? to = null, [FromQuery] int? salesExecutiveId = null, [FromQuery] int? projectId = null)
    {
        ...
        var employeeQuery = db.Users.AsNoTracking().Where(x => x.Role.Name == "SalesExecutive" || x.SalesTeamId.HasValue);
        ...
        var followUpOutcomes = await db.FollowUps.AsNoTracking().Where(x => x.CreatedAt >= start && x.CreatedAt < endExclusive && employeeIds.Contains(x.CreatedById) && (x.ResultingStatus == LeadStatus.Lost || x.ResultingStatus == LeadStatus.NotInterested) && (!projectId.HasValue || x.Lead.ProjectId == projectId)).Select(x => new { x.LeadId, ChangedById = x.CreatedById, ToStatus = x.ResultingStatus!.Value, x.Lead.ProjectId, ChangedAt = x.CreatedAt }).ToListAsync();
        outcomes = outcomes.Concat(followUpOutcomes).GroupBy(x => new { x.LeadId, x.ChangedById, x.ToStatus }).Select(x => x.OrderBy(v => v.ChangedAt).First()).ToList();
        ...
        var initialAssignments = await db.Leads.AsNoTracking().Where(x => x.AssignedAt >= start && x.AssignedAt < endExclusive && x.AssignedToId.HasValue && employeeIds.Contains(x.AssignedToId.Value) && (!projectId.HasValue || x.ProjectId == projectId)).Select(x => new { EmployeeId = x.AssignedToId!.Value, LeadId = x.Id, ChangedAt = x.AssignedAt!.Value }).ToListAsync();
        assignmentRows = assignmentRows.Concat(initialAssignments).GroupBy(x => new { x.EmployeeId, x.LeadId }).Select(x => x.OrderBy(v => v.ChangedAt).First()).ToList();
        ...
        var visibleGroupIds = employees.Where(x => x.GroupId.HasValue).Select(x => x.GroupId!.Value).Distinct().ToArray();
        var teamMasters = await db.SalesTeams.AsNoTracking().Where(x => !User.IsInRole("GroupLeader") || visibleGroupIds.Contains(x.SalesGroupId)).Select(x => new { x.Id, x.Name, x.SalesGroupId, Group = x.SalesGroup.Name }).OrderBy(x => x.Name).ToListAsync();
        var groupMasters = await db.SalesGroups.AsNoTracking().Where(x => !User.IsInRole("GroupLeader") || visibleGroupIds.Contains(x.Id)).Select(x => new { x.Id, x.Name }).OrderBy(x => x.Name).ToListAsync();
        var teamSummary = teamMasters.Select(team => { var ids = employees.Where(x => x.TeamId == team.Id).Select(x => x.Id).ToHashSet(); var rows = kpis.Where(x => ids.Contains(x.employeeId)).ToList(); return new { group = team.Group, team = team.Name, employees = ids.Count, averageKpi = rows.Count == 0 ? 0 : Math.Round(rows.Average(x => x.score), 2), won = rows.Sum(x => x.won), lost = rows.Sum(x => x.lost), returned = rows.Sum(x => x.returned), salesUnits = rows.Sum(x => x.salesUnits), collections = rows.Sum(x => x.collections) }; }).OrderByDescending(x => x.averageKpi).ToList();
        var groupSummary = groupMasters.Select(group => { var ids = employees.Where(x => x.GroupId == group.Id).Select(x => x.Id).ToHashSet(); var rows = kpis.Where(x => ids.Contains(x.employeeId)).ToList(); return new { group = group.Name, employees = ids.Count, teams = teamMasters.Count(x => x.SalesGroupId == group.Id), averageKpi = rows.Count == 0 ? 0 : Math.Round(rows.Average(x => x.score), 2), won = rows.Sum(x => x.won), lost = rows.Sum(x => x.lost), returned = rows.Sum(x => x.returned), salesUnits = rows.Sum(x => x.salesUnits), collections = rows.Sum(x => x.collections) }; }).OrderByDescending(x => x.averageKpi).ToList();
