...
public class PaymentsController(CrmDbContext db, IFinancialService financial, IOneSignalNotificationService push) : ControllerBase
{
    ...
    public async Task<ActionResult> Record(RecordPaymentRequest request,[FromHeader(Name="Idempotency-Key")]string? key)
    {
        ...
        var payment=new Payment{CustomerId=request.CustomerId,CollectionNumber=$"COL-{DateTime.UtcNow:yyyyMMddHHmmss}-{Guid.NewGuid():N}"[..31],SalesExecutiveId=customer.BookedById??customer.AssignedToId.Value,Amount=request.Amount,PaymentDate=request.PaymentDate.Date,Method=request.Method,Purpose=request.Purpose,TransactionReference=request.TransactionReference?.Trim(),InstallmentId=request.InstallmentId,ProofUrl=request.ProofUrl,Remarks=request.Remarks,Status=PaymentStatus.Approved,SubmittedById=User.UserId(),VerifiedById=User.UserId(),VerifiedAt=DateTime.UtcNow,IdempotencyKey=key};
